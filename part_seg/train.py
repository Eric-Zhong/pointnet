...
def train():
    with tf.Graph().as_default():
        ...
        def train_one_epoch(train_file_idx, epoch_num):
            ...
            for i in range(num_train_file):
                ...
                for j in range(num_batch):
                    ...
                    total_label_acc += np.mean(float(per_instance_label_pred == cur_labels[begidx: endidx, ...]))
        ...
        def eval_one_epoch(epoch_num):
            ...
            total_label_acc_per_cat = np.zeros((NUM_CATEGORIES)).astype(float)
            total_seg_acc_per_cat = np.zeros((NUM_CATEGORIES)).astype(float)
